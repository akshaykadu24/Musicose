...
export const addCart = (item)=>async(dispatch)=>{
   ...
   dispatch({type:CART_LOADING})
    ...
    axios({
        method:"POST",
        url:`${MusicoseApi}/cart/create/${item._id}`,
        data:item,
        headers:{
...
console.log(userid)
    dispatch({type:CART_LOADING})
    try{
        let res = await axios.get(`${MusicoseApi}/cart`,{
            headers:{
                "Content-Type" : "application/json",
                "Authorization" : JSON.parse(localStorage.getItem("token"))
             },
...
export const deleteCart = (id)=>async(dispatch)=>{
    ...
    dispatch({type:CART_LOADING})
    try{
        let res = await axios.delete(`${MusicoseApi}/cart/delete/${id}`,{
            headers:{
                "Content-Type" : "application/json",
                "Authorization" : JSON.parse(localStorage.getItem("token"))
             },
...
export const updateCart = (id,quantity)=>async(dispatch)=>{
    ...
    dispatch({type:CART_LOADING})
    // try{
    //     let res = await axios.patch(`${MusicoseApi}/cart/update/${id}`,{
    //         headers:{
    //             "Content-Type" : "application/json",
    //             "Authorization" : JSON.parse(localStorage.getItem("token"))
    //          },
    ...
    axios({
        method:"PATCH",
        url:`${MusicoseApi}/cart/update/${id}`,
